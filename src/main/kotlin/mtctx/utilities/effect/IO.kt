 ...
 * @see Effect
 * @see mtctx.utilities.Outcome
 */
open class IO<A>(thunk: suspend () -> A) : Effect<A>(thunk) {
     ...
     */
    override fun <B> map(transform: suspend (A) -> B): IO<B> = IO { transform(thunk()) }
     ...
     */
    override fun <B> flatMap(transform: suspend (A) -> Effect<B>): IO<B> = IO {
     ...
     */
    override fun <B> ap(function: Effect<(A) -> B>): IO<B> = IO {
    ...
    companion object : Effect.Companion<IO<*>> {
         ...
         */
        override fun <A> pure(value: A): IO<A> = IO { value }
    }
}
 ...
 */
fun <A> io(thunk: suspend () -> A): IO<A> = IO(thunk)
fun <A> io(value: A): IO<A> = IO.pure(value)