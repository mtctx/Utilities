...
sealed interface Outcome<out T> {
     ...
     */
    @JvmInline
    value class Success<T : Any>(val value: T) : Outcome<T> {
        /**
         * Returns a string representation of this [Success] instance.
         *
         * @return A string in the format "Success(value=$value, type=$type)".
         */
        override fun toString(): String = "Success(value=$value, type=${value::class.simpleName ?: "UnknownType"})"
    }
    ...
    data class Failure(
        ...
        override fun toString(): String = buildString {
            append("Failure(message=$message")
            throwable?.let {
                append(", cause=${it::class.simpleName}")
                append(", stacktrace=${it.stackTraceToString()}")
            }
            outcome?.let { append(", nested=$outcome") }
            append(")")
        }
    }
...
fun Outcome<*>.succeeded(): Boolean {
    contract {
        returns(true) implies (this@succeeded is Success<*>)
        returns(false) implies (this@succeeded is Failure)
    }
 ...
 */
suspend inline fun <T, R : Any> Outcome<T>.map(transform: suspend (T) -> R): Outcome<R> = when (this) {
 ...
 */
suspend inline fun <T, R : Any> Outcome<T>.mapCatching(transform: suspend (T) -> R): Outcome<R> = when (this) {
    is Success ->
        try {
 ...
 */
suspend inline fun <T : Any> Outcome<T>.recover(recoverBlock: suspend (Failure) -> T): Outcome<T> = when (this) {
    is Success -> this
 ...
 */
suspend inline fun <T : Any> Outcome<T>.recoverCatching(recoverBlock: suspend (Failure) -> T): Outcome<T> =
    when (this) {
        is Success -> this
        is Failure ->
            try {
 ...
 */
fun <T : Any> Result<T>.toOutcome(): Outcome<T> = fold(::success, ::failure)
 ...
 */
suspend inline fun <T : Any> runCatchingOutcomeOf(block: suspend () -> T): Outcome<T> =
    try {
        success(block())
    } catch (e: Throwable) {
        failure(e)
 ...
 * @return A [Success] with the result of [block], or a [Failure] if an exception occurs.
 */
suspend inline fun <T : Any> T.runCatchingOutcomeOf(block: suspend () -> T): Outcome<T> =
    try {
        success(block())
    } catch (t: Throwable) {
 ...
 */
suspend inline fun <T : Any> success(block: suspend () -> T): Success<T> {
